...
window.addEventListener('message', (event) => {
    ...
    // Zoom controls from iframe
    // if (event.data.action === 'zoomIn') {
    //     // Create a wheel event to trigger zoom in
    //     const wheelEvent = new WheelEvent('wheel', { 
    //         deltaY: -1, 
    //         bubbles: true 
    //     });
    //     document.dispatchEvent(wheelEvent);
    //     console.log('Zoom in from iframe');
    // }
    
    // if (event.data.action === 'zoomOut') {
    //     // Create a wheel event to trigger zoom out
    //     const wheelEvent = new WheelEvent('wheel', { 
    //         deltaY: 1, 
    //         bubbles: true 
    //     });
    //     document.dispatchEvent(wheelEvent);
    //     console.log('Zoom out from iframe');
    // }
    
    if (event.data.action === 'zoomToggle') {
        // Call the toggle zoom function directly
        if (typeof toggleZoom === 'function') {
            toggleZoom();
        }
        console.log('BufferSet: Zoom toggle from iframe');
    }
    
    if (event.data.action === 'zoomOut') {
        // Reset zoom to normal level
        const wheelEvent = new WheelEvent('wheel', { 
            deltaY: 100, // Large positive value to ensure zoom out to level 0
            bubbles: true 
        });
        document.dispatchEvent(wheelEvent);
        console.log('BufferSet: Zoom reset from iframe');
    }
});
