...
function copyAssets() {
    return new Promise((resolve, reject) => {
        // Copy MP3 file to public directory
        try {
            fs.copyFileSync('src/battleThemeA.mp3', 'public/battleThemeA.mp3');
            resolve();
        } catch (error) {
            reject(error);
        }
...
function scheduleCompile() {
    ...
    watchTimerId = setTimeout(() => {
        ...
        copyAssets().then(() => compile()).then(() => createZip()).then(() => console.log('done'));
